...
- **Key surfaces / files:**
  ...
  - `session-store-and-upload-api`: `src/tools/session_store.py`, `src/domain/session.py`, `src/api/sessions.py` (its `POST` handler generically forwards whatever `warnings` list `file-parsing`'s parse function returns — always `[]` in Phase 1 — so Phase 2's `malformed-file-handling` slice only has to extend the parser, never this file), `tests/unit/api/test_sessions.py`
  - `qa-graph-core`: `src/graph/state.py`, `src/graph/nodes.py`, `src/graph/edges.py`, `src/graph/agent.py`, `src/tools/code_executor.py`, `src/tools/exec_worker.py`, `src/tools/privacy_guard.py`, `src/prompts/codegen.md`, `src/prompts/interpret.md`, `src/llm/providers/gemini.py` (extended), `src/config/settings.py` (adds `AGENT_LLM_MODEL_CODEGEN`, `AGENT_LLM_MODEL_INTERPRET`, `AGENT_MAX_RETRIES`, `AGENT_EXEC_TIMEOUT_SECONDS` — owned solely by this slice since only the graph itself consumes them), `tests/unit/graph/test_agent.py`, `tests/unit/tools/test_privacy_guard.py`
  - `questions-api-and-wiring`: `src/api/questions.py` (builds its response by generically forwarding every `QAState` output field — including `cost_estimate_usd`/`chart_spec`/`table_data`, which simply stay `null` until Phase 2's `phase2-integration` slice populates them in state, so Phase 2 never needs to touch this file again), `src/domain/question.py`, `src/observability/events.py` (extended), `src/graph/runner.py` (extended), `tests/integration/test_phase1_qa.py`, `tests/fixtures/sales_1000.csv`
...
- **Independent slices (parallel build units):** the first four are pure, self-contained additions that do not touch the graph or the questions endpoint, so they build fully in parallel; the fifth slice is the only one that wires their outputs together and is declared dependent on all four.
  - `conversation-memory-store` (backend) — extend the session store with a conversation-turn list and an append method; extend `GET /api/sessions/{session_id}` (a thin passthrough of session-store fields) to return it. Deps: none.
  ...
  - `phase2-integration` (backend) — the ONLY slice touching `graph/nodes.py`, `graph/state.py`, and `prompts/*.md`: threads `conversation_history` into both prompts, and has `finalize` call the cost estimator and chart-spec derivation and append to the session's conversation history and the JSONL log. It does NOT touch `api/questions.py` — Phase 1's endpoint already forwards every `QAState` output field generically, so populating these fields in state is sufficient for them to appear in the API response. Deps: `conversation-memory-store`, `cost-and-logging`, `malformed-file-handling`, `chart-spec-derivation` (calls all four's functions) — this slice is serialized after the other four land.
...
- **Key surfaces / files:**
  - `conversation-memory-store`: `src/tools/session_store.py`, `src/domain/session.py`, `src/api/sessions.py` (extend the `GET` handler to return the new `conversation_history` field)
  ...
  - `phase2-integration`: `src/graph/state.py`, `src/graph/nodes.py`, `src/prompts/codegen.md`, `src/prompts/interpret.md`, `tests/integration/test_conversation_memory.py`
